...
class Cell_Patch(Patch):
    ...
    def neighbors_alive(self):
        total = self.neighbors((-1,0)) * 4 + (self.is_alive * 2) + self.neighbors((0,-1))


    def set_alive_or_dead(self, alive_or_dead: bool):
        self.is_alive = alive_or_dead
        self.set_color(Cell_Patch.fg_color if self.is_alive else Cell_Patch.bg_color)
...
class Life_World(World):
    ...
    def get_colors(self):
        Cell_Patch.bg_color = self.get_color_and_update_button(
            self.bg_color_chooser,
            default_color_string=rgb_to_hex(Cell_Patch.bg_color))
        Cell_Patch.fg_color = self.get_color_and_update_button(
            self.fg_color_chooser,
            default_color_string=rgb_to_hex(Cell_Patch.fg_color))
    ...
    def handle_event_and_values(self):
        ...
        default_color_string = rgb_to_hex(Cell_Patch.fg_color if foreground else Cell_Patch.bg_color)
